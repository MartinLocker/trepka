...
void MFRC522::PCD_Reset() {
	...
	uint8_t count = 0;
	do {
		// Wait for the PowerDown bit in CommandReg to be cleared (max 150x1ms)
//		delay(1);
	} while ((PCD_ReadRegister(CommandReg) & (1 << 4)) && (++count) < 150);
} // End PCD_Reset()

void MFRC522::PCD_PowerDown() {
	PCD_WriteRegister(CommandReg, PCD_NoCmdChange | 0x10);
}	

void MFRC522::PCD_SetToReadWrite(bool Write) {
	...
	if (Write) {
		...
		PCD_WriteRegister(TReloadRegH, 0x00);		// Reload timer with 0x00C8 = 200, ie 5ms before timeout.
		PCD_WriteRegister(TReloadRegL, 0xC8);
	} else {
...
//		PCD_WriteRegister(TReloadRegL, 0x1E);		// ZVAZIT PRODLOUZENI ???
		PCD_WriteRegister(TReloadRegL, 0x3C);		// ZVAZIT PRODLOUZENI ???
//		PCD_WriteRegister(TReloadRegL, 0x68);		// ZVAZIT PRODLOUZENI ???
	}
...
void MFRC522::PCD_PowerUpReset(bool hardReset) {
	if (!hardReset) {
		PCD_Reset();
	}
	PCD_SetToReadWrite();
...
void MFRC522::PICC_DumpMifareUltralightToSerial() {
	...
	for (byte page = 0; page < 36; page +=4) { // Read returns data for 4 pages at a time.
		// Read pages
		byteCount = sizeof(buffer);
