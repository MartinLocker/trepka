...
uint16_t Store::eepromCounter; // pocet zapisu do eeprom (protoceni zaznamu)
...
bool Store::readConfig()  {
  if (config.sn == 0) {
    ...
    readEEPROM(EEPROM_COUNTER, (uint8_t*)&eepromCounter, sizeof(eepromCounter));
    if (eepromCounter == 0xFFFF) eepromCounter = 0;
    return 1;
  }
  return 0;
}
...
//  addrActual = (addrActual + 1) % EEPROM_TOP;
  addrActual++;
  if (addrActual == EEPROM_TOP) {
    addrActual = 0;
    // zvys pocitadlo zapisu do eeprom
    eepromCounter++;
    writeEEPROM(EEPROM_COUNTER, (uint8_t*)&eepromCounter, sizeof(eepromCounter));
  }
  return 1;
}
