...
/* cas z internich hodin
  time_t t;
  time(&t);
  return (t + Store::config.timeZone*3600 - SECONDS_FROM_1970_TO_2000 - SECONDS_FROM_2000_TO_2020);
*/
  return (getRTCTime() + Store::config.timeZone*3600 - SECONDS_FROM_1970_TO_2000 - SECONDS_FROM_2000_TO_2020);  
}

// Vraci cas v sekundach z externiho RTC bez casoveho pasma vuci od 1.1.1970 00:00:00 UTC
...
char* RTC::printTime(char* s, uint32_t time, bool unix) {
  DateTime(time + (unix ? Store::config.timeZone*3600 : SECONDS_FROM_1970_TO_2000 + SECONDS_FROM_2000_TO_2020)).toString(s);
  return s;
}